...
def save(tweet)
  puts tweet.id
  bin = tweet.id.to_s.slice(0, tweet.id.to_s.length - 15)
  FileUtils.mkdir_p(File.join(data_dir, bin))
  File.open(File.join(data_dir, bin, tweet.id.to_s), 'wb') {|f| f.write(tweet.attrs.to_json) }
end
